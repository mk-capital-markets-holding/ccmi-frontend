...
export default function Header({ onSearchOpen }) {
  ...
  useEffect(() => { setMobileOpen(false); setOpenMega(null); }, [loc.pathname]);
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all ${scrolled ? "bg-mk-ink/95 backdrop-blur-sm" : "bg-mk-ink"} text-white border-b border-white/5`}
    ...
    >
      <div className="container-mk flex items-center justify-between h-16 md:h-20">
        ...
        <nav className="hidden lg:flex items-center gap-7">
          {MAIN_LINKS.map((l) => (
            <div key={l.to} className="relative" onMouseEnter={() => l.mega && setOpenMega(l.mega)} onMouseLeave={() => setOpenMega(null)}>
              <Link
                to={p(l.to)}
              ...
              {l.mega && openMega === l.mega && (
                <div className="absolute left-1/2 -translate-x-1/2 top-full pt-3">
                  <div className="w-[720px] bg-mk-ink2 border border-white/10 p-8 grid grid-cols-2 gap-x-10 gap-y-4 shadow-2xl mk-grain" data-testid={`mega-${l.mega}`}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={onSearchOpen} data-testid="btn-search-open" aria-label="Open search" className="p-2 text-white/80 hover:text-mk-bronze transition-colors">
            <Search className="w-4 h-4" strokeWidth={1.5} />
          </button>
          <button onClick={toggleLang} data-testid="btn-lang-toggle" className="flex items-center gap-1.5 text-xs uppercase tracking-widest text-white/80 hover:text-mk-bronze transition-colors px-2 py-1">
            <Globe className="w-3.5 h-3.5" strokeWidth={1.5} />{lang}
          </button>
          <Link to={p("/contact")} className="hidden md:inline-block bg-mk-bronze text-mk-ink text-sm px-4 py-2.5 font-medium hover:bg-mk-bronze2 transition-colors" data-testid="btn-header-demo">
            {t("cta.demo")}
          </Link>
          <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 text-white" data-testid="btn-mobile-menu">
      ...
      {mobileOpen && (
        <div className="lg:hidden bg-mk-ink border-t border-white/10 mk-grain" data-testid="mobile-menu">
          <div className="container-mk py-6 flex flex-col gap-4">
            {MAIN_LINKS.map(l => (
              <Link key={l.to} to={p(l.to)} className="text-white/90 py-2 border-b border-white/5">{t(l.key)}</Link>
            ))}
            <Link to={p("/contact")} className="text-white/90 py-2 border-b border-white/5">{t("nav.contact")}</Link>
            <Link to={p("/founder")} className="text-white/90 py-2 border-b border-white/5">{t("nav.founder")}</Link>
            <Link to={p("/contact")} className="bg-mk-bronze text-mk-ink text-sm px-4 py-3 text-center mt-2">{t("cta.demo")}</Link>
          </div>
        </div>
      )}
    </header>
