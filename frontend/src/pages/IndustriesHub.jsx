...
export default function IndustriesHub() {
  ...
  return (
    <>
      <SEO title={t("nav.industries")} description={t("ind.lede")} path="/industries" />

      <section className="bg-mk-ink text-white pt-20 pb-24 mk-grain">
        <div className="container-mk">
          <Breadcrumbs items={[{ label: t("bc.industries") }]} />
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-8">
              <div className="overline mb-4">{t("ind.overline")}</div>
              <h1 className="font-serif text-5xl md:text-7xl leading-[1.02]">{t("ind.title")}</h1>
            </div>
        ...
        <div className="container-mk grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-mk-line/15 border border-mk-line/15">
          {INDUSTRIES.map((ind) => (
            <Link key={ind.slug} to={p(`/industries/${ind.slug}`)} className="group bg-mk-paper p-10 hover:bg-white transition-colors" data-testid={`ind-hub-${ind.slug}`}>
              ...
              <h3 className="font-serif text-2xl md:text-3xl">{L(ind.name, lang)}</h3>
              <p className="text-mk-text2 mt-3 leading-relaxed">{L(ind.lede, lang)}</p>
              <div className="mt-6 border-t border-mk-line/15 pt-4 flex justify-between text-sm">
                <span className="text-mk-bronze2 group-hover:translate-x-1 transition-transform font-mono uppercase tracking-widest">{t("cta.read")}</span>
                <span className="text-mk-text2">{ind.relevantModules.length} {t("ind.count_modules")}</span>
              </div>
            </Link>
          ))}
        </div>
